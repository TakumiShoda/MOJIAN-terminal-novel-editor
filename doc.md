...
| `Ctrl+S` | 保存 |
| `Ctrl+Shift+S` | 打快照（可加标签）—— **传统键盘模式下到不了**，见下方注 |
| `F9` | 打快照（实际可用的入口） |
| `Ctrl+B` | 切换目录树 |
| `Ctrl+F` / `Ctrl+H` | 查找 / 查找替换 |
| `Ctrl+Shift+F` | 全书查找 |
| 查找替换内：`F4` 切换作业范围（当前章 / 当前卷 / 全书） |  |
| 查找替换内：`Alt+A` 替换（当前章＝按勾选；全卷/全书＝该范围全部，先弹确认） |  |
| `Alt+U` | 撤销刚才那次批量作业（全卷/全书排版或替换，一次性回滚受影响章节） |
| `F5` | 一键排版（当前章，弹预览）；预览内 `V`/`B` 施加到当前卷/全书 |
| `F7` | 校对当前章 |
| `F8` | 历史面板 |
| `Alt+C` | 角色速查侧栏 |
| `Ctrl+Z` / `Ctrl+Y` | 撤销 / 重做 |
...
> **注（实现时发现）**：`Ctrl+Shift+S` 在**传统键盘模式**下根本到不了——终端对
> `Ctrl+S` 与 `Ctrl+Shift+S` 发的是同一个字节（0x13），Shift 压根没编码进去。
> 要区分得开 Kitty 键盘协议（§2.3：可选，需运行时探测），那是 M6 的内容。
